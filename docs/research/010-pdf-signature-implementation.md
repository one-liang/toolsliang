...
| 上限 | 100 頁、50 MiB，並依 `navigator.deviceMemory` 只下修不上修；工作集估算超過預算即停止 | 規格 §12.12「subject to device capability」、009 §9.2 |
| 匯入的簽名圖片 | 完全不透明的圖片直接退回，並說明它會遮住頁面 | 009 §6.5 要求 alpha；與其說默默壓上一個白框，不如讓使用者換一張 |

## 2. 兩個引擎怎麼進到瀏覽器

`pdfjs-dist@6.3.289`（Apache-2.0）與 `@cantoo/pdf-lib@2.9.2`（MIT）都以 npm 相依套件的形式打包進 `pdf-signature.worker.ts` 的 chunk，不另外提交任何第三方檔案到版本控制。建置後的 chunk 為 2,306,222 bytes，brotli（品質 5）後 679,587 bytes，與 009 §9.1 估算的約 696 KiB 相符。這個 chunk 只在開啟工具頁時才會被取得，Landing Page 與其他工具不會載入它。
...
- 寫入引擎在開檔階段也要試著開一次（009 §9.3 第 4 點）。兩個引擎接受的文件並不完全相同，只有預覽引擎開得起來的文件若等到匯出才失敗，使用者已經排好簽名了。這次開啟的文件立刻丟棄，匯出時再從原始位元組重新開啟，因此重複匯出不可能蓋兩次。
- 一次拖曳會產生數十個 `pointermove`，每一個都寫一筆復原紀錄的話，「復原」只會退回一個取樣點。改成一次手勢只記一步，並為復原紀錄設上限。
...
- 代表性 20 頁 20 MiB 文件：首頁預覽最慢 49 毫秒（預算 3,000）、匯出最慢 36 毫秒（預算 15,000）。
