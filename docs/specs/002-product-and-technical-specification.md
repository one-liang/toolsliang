...
- NEW has explicit `startsAt` and `endsAt`; after expiry it disappears automatically. Because public surfaces are prerendered, NEW is evaluated once at hydration against the visitor's device date and never against the build date: prerendered HTML carries no date-derived label, the label appears as the page becomes interactive without moving the layout around it, and a page left open keeps the label it was given until it reloads. HOT is an editorial or aggregate non-personalized flag with a documented source. SAVED comes from the user's common-tool state. PRO remains presentational only until a separate accepted product and access-control decision exists.
...
- **Version one scope:** Three narrowings are deferred, all recorded in `docs/research/003-random-picker-fairness-and-sources.md`. No draw history is kept (§9), because a retained record would suggest a draw can be checked afterwards, which the no-audit caveat denies; the local export the boundary rule below allows is deferred with it, so a result lives only until the tab closes. Parsing shows no progress (§4.4): the entry ceiling parses far inside the performance budget below, where a bar would flash rather than inform, so the list summary is the completion signal instead. The wheel is drawn as SVG rather than canvas (§9), so sectors and labels are readable DOM nodes and no separate equivalent text has to be maintained.
...
- **Route and slug:** `/{locale}/tools/random-picker/`; stable slug `random-picker`.
...
- **Accessibility:** The form and text-result flow is complete without wheel/drag. The wheel has an adjacent candidate list, result live region, skip-animation control, and reduced-motion instant result.
