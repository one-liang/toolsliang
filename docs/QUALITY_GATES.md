...
- 商品圖工作台：以工具自己的 spec 驗證跨引擎的作業流程。三瀏覽器驗證略過去背的合規主圖流程（產生 1000 × 1000 JPEG、容量落在通路範圍內、規則核對結果與免責同時可見、下載檔名不帶來源檔名）、品牌宣傳圖分支（版型結果再疊上 Logo，讀回 PNG 的角落與中心像素確認圖層順序）、取消與失敗只影響該步驟（被拒絕的檔案不推翻既有匯入、算繪中途取消後先前步驟仍為已完成、回到前一步修改設定會撤下舊輸出）、去背引擎不可用時只停用該步驟、純鍵盤切換步驟時焦點與進度隨之移動、未完成工作會攔截離開，以及清空工作台會撤銷輸出的 Blob URL。中英文頁各驗證一次 canonical／hreflang、被拒絕輸入的訊息語言、320–1440px reflow 與亮暗模式 axe。完整代表性流程（下載模型、本機去背、接續版型與下載）只在 Chromium 執行一次，理由與人像去背相同。工作階段狀態機、每步驟錯誤字彙與場景組裝由單元測試負責，不在瀏覽器重複驗證。12 MP 的效能預算由合規主圖與圖片壓縮的 spec 量測，工作台沿用同一組引擎，不重複量測。
...
商品圖工作台專屬驗證位於 `tests/product-image-workbench-session.test.ts`、`-pipeline.test.ts`、`-content.test.ts`、`-workspace.test.ts`、`-privacy.test.ts`、`tests/unload-guard.test.ts` 與 `tests/e2e/product-image-workbench.spec.ts`。工作台不新增算繪程式，它組合人像去背、合規主圖與品牌宣傳圖三個既有引擎，因此像素層級的正確性仍由那三個工具自己的 spec 負責。這裡驗證的是組合本身：工作階段狀態機保證重新執行某一步會作廢它之後的成果、失敗與取消只落在該步驟、不適用與能力不足的步驟不阻擋後續步驟，且工作階段文件只有步驟狀態，不含檔名、Blob 或像素；場景組裝驗證版型結果在品牌步驟不被二次縮放，且合規分支不會帶著通路容量規則跑到宣傳輸出上。取消需要一個還在進行中的步驟。延遲引擎自己的 Worker 程式回應無法製造這個狀態——Service Worker 與瀏覽器快取都會直接回應那次請求，攔截不到——因此改以真正繁重的工作製造：5.8 MP 來源寫入工具允許的最大畫布，並以 `force` 點擊取消，不等按鈕在重排中靜止。合成素材全部由程式繪製，不使用真實商品照或人像照。
