...
export function createImageCompressor() {
  return createWorkerEngine<CompressionInput, CompressionWireOutput, CompressionOutput>({
    ...
    async validate(input) {
      if (!['image/png', 'image/jpeg', 'image/webp'].includes(input.format) || !Number.isFinite(input.quality) || input.quality < 0 || input.quality > 1 || ![input.maxWidth, input.maxHeight].every(value => Number.isInteger(value) && value > 0 && value <= 8192)) return 'invalid_options'
      const error = await validateImageInput(input.file)
      if (error) return error
      if (input.file.size > 25 * 1024 * 1024) return 'too_large'
    },
