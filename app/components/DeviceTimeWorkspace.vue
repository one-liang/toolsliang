...
<template>
  <Card class="tool-workspace device-time">
    ...
    <p role="status" class="field-help" :class="{ 'field-error': copyState === 'failed' }">{{ message }}</p>
  </Card>
</template>
...
.device-time__clock { display: block; color: var(--color-foreground); font-family: var(--font-mono); font-size: clamp(2rem, 4vw, 3.5rem); font-weight: 750; line-height: 1.35; }
